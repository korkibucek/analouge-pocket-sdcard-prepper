...
### Added
- **Folder picker for ROM sources** (closes #116): the web UI ROM step has a **Browse…** button that opens a modal to navigate the local filesystem (drives/home, up, into folders) and pick the source folder. Backed by `Get-PocketDirectoryListing` + read-only token-gated `POST /api/browse` (lists folder names only).
- **Full community core inventory** (closes #113): `manifests/cores.json` is now generated from the entire openFPGA Cores Inventory (~51 cores) by `scripts/Update-CoresManifest.ps1` (+ a monthly refresh workflow that opens a PR). New `Install-PocketCoreSet` installs every core in one action (each unique repo once; `-DryRun`; graceful rate-limit/failure handling), wired to `POST /api/cores/install-all`, an "Install ALL cores" web button, and a CLI prompt. Added `Install-PocketCore -SkipIdentifierCheck` for repos whose folder name differs from the inventory identifier (e.g. GB-GBC).
